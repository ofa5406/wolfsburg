...
*Last updated: 2026-08-13 (**InfAU final submission package built and verified — `final submission/`, due Aug 14.** The hand-in is a folder uploaded to Nextcloud `02_StadtHub/`; `site/` is what the university republishes. Saved the loose work first: the hub-viewer transparent-PNG export (pushed to `main`, `dbb5156`) and the tier-table vector-export branch, which **existed only on this laptop** and is now pushed. The real work was making the **activity map run with no internet** — the deck, brain and hub-viewer already passed. ~20 live Overpass queries became **26 captured snapshots** (`public/osm/`, 65 MB, regenerate with `scripts/capture_osm_snapshots.mjs`) read through `src/osmSnapshot.js`; the Carto basemap and MapLibre demo glyph server became `src/mapBaseStyle.js` + `public/glyphs/`; Esri satellite tiles and an unused Google Fonts link removed; `base` → `'./'`. **District boundaries were the critical one** — they load on first render, so offline the map drew no districts at all. The package is assembled by scripts, not by hand: `build-site.mjs` (deck → `site/index.html`, **map embeds rebuilt from source** because the committed ones still called Overpass, deck's `../../` paths + three hard-coded GitHub URLs rewritten into links to the three sub-apps, embeds pruned to the data they name, `.nojekyll`), `downsize-media.mjs` (119 MB → 20 MB), and three checkers. **`verify-offline.mjs` blocks every outbound request and serves the folder one level deep — all four pieces clean, 0 external requests, 0 404s.** `site/` = 248 MB (limit 1 GB). **Still needed from the team: photographs of the Summaery exhibit and the physical model, printed posters/boards, the Rhino `.3dm` files, a contact email that outlives graduation, and the historical photo credits** — `materials/` and `exhibition/` are empty and the README marks the two gaps openly. See `sessions/2026-08-13_final-submission/notes.md`.)*

*Previous update: 2026-07-09 (**Exhibition-day polish pass on `exhibition/deck/` — done + pushed (`086db7d`), the night Summaery opens.** Verified headless in real Chrome after each change (0 exceptions / 404s / broken images), ~20 commits pushed to `ofa5406/wolfsburg` `main`. Highlights: reading-time **pacing floor** (no slide leaves before it's readable); **4.2** dot-matrix sweeps all rows together; **before/after wipes** (4.3, 6.2–6.4) gated on image decode + slowed (before 1s · wipe 2.4s · after 4s); **5.2** animated S/M/L traffic arcs (all 6 pairs); **3.1** now one arrow per bullet, word-right→bullet-left; **brain (8.2)** nodes smoothed + category chips highlight in accent (not hide/show), doubled corner frames removed in kiosk; **8.1/8.2** iframes inset off the L's; **1.3 Today REBUILT** to the 1.2 gallery (STORAGE/MOVEMENT/SEPARATION, 6 new photos in `assets/today/`, PLACEHOLDER badges gone); **8.3 close** = full-bleed masterplan + 3 scan-verified QR codes (site + both repos) + activity-map link; **hubembed (6.2–6.4) rebuilt** (element buttons keep names + stay round, description moved below); **5.4** map zoomed out 11.5→10.55 to match 2.1; real `< >` title brackets; em dashes gone from visible prose; **bottom chrome aligned to the corner L's** (Present button nested inside the bottom-left L, counter enlarged). **NEW `deck/start-exhibition.cmd`** — offline kiosk launcher; the deck MUST be served (opening index.html as a file leaves the ES-module embeds blank). **Fleet number ruled out of scope by the team — do not raise it in deck work.** Open: watch a full loop on the exhibition PC (screen-sleep off) + scan the QR codes; `assets/history/h1-map.png` (7.9MB) is the last heavyweight, downscale if the loop stutters; the hubembed source (`wolfsburg-activity-map/embed-hub/`) is untracked upstream — team to PR it. See `sessions/2026-07-09_exhibition-polish/notes.md`.)*

*Previous update: 2026-07-07 (**Full 24-page exhibition presentation BUILT + DEPLOYED.** Grew `exhibition/deck/` from 5 slides into the complete self-running Summaery pitch-talk per `exhibition/PRESENTATION-OUTLINE.md` (refined + completed this session from the print text + hub_typologies + decisions). 7 new `deck.css` template variants (T2.1 today · T2.2 potential · T-STAT outcomes · T-DIAGRAM S/M/L + hierarchy · T-TYPO S/M/L sheets · T-PLAN masterplans · T-STATEMENT manifesto) + bracket titles / ghost numbers / accent / before-after wipe. `deck.js` is now a **24-slide array** with a generic `txtSlide` builder + counter animation, and map/hub/brain are found by slide **`kind`** (not hardcoded index) so autoplay/loop/idle-resume survive the longer deck. All prose typed; live map + 3D hub-viewer + brain graph embedded (click-to-activate, GPU pause off-screen). Curated M/L-hub aerials copied to `exhibition/deck/assets/`. **The deck is now the live homepage** (root `index.html` redirect → `exhibition/deck/`). Headless-verified (24 slides, 0 console/exception errors, no broken images); committed + pushed `ofa5406/wolfsburg` `main` `ca34c09`; **live at ofa5406.github.io/wolfsburg/**. Open: ⚠ **fleet number 763 vs 1,300** (§4.2 shows 763 + a visible TODO tag), S-hub uses a street-scene stand-in, pacing pass + hi-res history images pending. See `sessions/2026-07-07_kiosk-presentation/notes.md`.)*
...
00. **InfAU final submission — due AUG 14 (tomorrow as of this update).** The
...
0. **Summaery exhibition (July 9–12) — deck POLISHED + LIVE.** `exhibition/deck/` is the exhibit + site homepage, deployed at ofa5406.github.io/wolfsburg/ (`086db7d`). The 2026-07-09 polish pass is done (pacing, animations, 1.3 rebuild, 8.3 QR close, hubembed rebuild, chrome alignment — see the session log). **On the exhibition PC:** run **`exhibition/deck/start-exhibition.cmd`** (serves locally + opens Chrome kiosk, offline — the deck must be *served*, not opened as a file), turn **screen-sleep off**, watch one full unattended loop, and scan the 8.3 QR codes from a phone. If the loop stutters, downscale the one remaining heavyweight `exhibition/deck/assets/history/h1-map.png` (7.9 MB). **Maps:** `mapembed` rebuilds from `wolfsburg-activity-map/embed/main.jsx` via `npx vite build --config vite.embed.config.js`; `hpmapembed`/`fleetembed` are orphaned builds (no source) — edit their baked `__DECK_MAP_VIEW` literal in the minified bundle. **hubembed** rebuilds from `wolfsburg-activity-map/embed-hub/` (untracked upstream; snapshot in `exhibition/deck/hubembed/_source/`) via `npx vite build --config vite.hubembed.config.js` — team should PR that source + the `hubElements.jsx` em-dash patch through the activity-map branch workflow, **not** push to its `master` (auto-deploys the live tool). ⚠ **Fleet number (763 vs 1,300) is out of scope for deck work** — the team settles it separately; do not raise it in the presentation.
