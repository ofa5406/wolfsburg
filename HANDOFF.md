...
0. **Summaery exhibition deck (July 9–12) — the live artefact is now `exhibition/deck/`** (the older `exhibition/kiosk/` is superseded but kept). Serve the wolfsburg root (`python -m http.server`) → open `exhibition/deck/index.html` (F11) for a live pass: smoothness, click-to-activate on the maps + hub, per-image history captions. Then: reconcile copy/captions with the team's Miro board, swap the low-res placeholder history images for hi-res, and set up the exhibition PC (Chrome `--kiosk`, screen-sleep off). **To change the maps:** edit the 3 landing sections / `ExportControl` / `embed/main.jsx`, then rebuild — `cd wolfsburg-activity-map && npx vite build --config vite.embed.config.js` and re-prune. Decide the commit/push story (deck location; activity-map embed → PR to `master`; hub-viewer render-loop edit). Optional: vendor maplibre glyphs for offline map labels; hub tour dwell 6 s → 5 s.
